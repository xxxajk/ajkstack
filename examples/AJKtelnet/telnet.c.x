...
/* IMPORTANT: initialize the network! */
	...
	if(!result) {
		...
		if(sock > -1) zap = sockconnect(sock, port, (unsigned char *) &destination[0]);
	}
	if(zap > 0) {
		printf("\nConnected\n");
		iodata[0] = 0xff;
		...
		sockwrite(sock, iodata, 3); /* Supress go ahead */
	} else {
		...
		*/
		...
		if(sock) {
			if(!smack) {
				...
				sockwrite(sock, iodata, 6);
			}
			/* lame attempt at input from keyboard */
			if(fixkbhit()) {
				again = fixgetch();
				xiodata[0] = again & 0xff;
				zap = sockwrite(sock, &xiodata[0], 1);
				if(xiodata[0] == 0xff) zap = sockwrite(sock, &xiodata[0], 1);
				if(zap < 0) {
					sock = -1;
				}
			}
